...
int nfr4x_branding_is_compatible(const char* base_dir)
{
	...
	nfr4x_log(LOG_DEBUG, "%-33s: processing %s", __FUNCTION__, base_dir);
	sprintf(brand_oem_cmd, "%s %s /usr/lib/enigma2/python brand_oem", nfr4x_PYTHON_BIN, nfr4x_BRANDING_HELPER_BIN);
	...
	if (fd) {
		...
		pclose(fd);
		nfr4x_log(LOG_DEBUG, "%-33s: brand_oem = %s", __FUNCTION__, brand_oem);
	}

	sprintf(box_type_inflash_cmd, "%s %s /usr/lib/enigma2/python box_type", nfr4x_PYTHON_BIN, nfr4x_BRANDING_HELPER_BIN);
	...
	if (fd) {
		...
		pclose(fd);
		nfr4x_log(LOG_DEBUG, "%-33s: box_type_inflash = %s", __FUNCTION__, box_type_inflash);
	}
	
	sprintf(box_type_cmd, "%s %s %s/usr/lib/enigma2/python box_type 2>/dev/null", nfr4x_PYTHON_BIN, nfr4x_BRANDING_HELPER_BIN, base_dir);
	...
	if (fd) {
		...
		pclose(fd);
		nfr4x_log(LOG_DEBUG, "%-33s: box_type = %s",__FUNCTION__,  box_type);
	}

	//fix for buggy oe-branding support of some image
	if (strlen(box_type) != 0 && !strcmp(brand_oem,"vuplus") && strncmp(box_type,"vu",2)) {
		nfr4x_log(LOG_DEBUG, "%-33s: buggy image... box_type:%s is invalid", __FUNCTION__, box_type);
		char buffer[255];
		strcpy(buffer,box_type);
		sprintf(box_type, "vu%s",buffer);
		nfr4x_log(LOG_DEBUG, "%-33s: patched box_type:%s should be ok", __FUNCTION__, box_type);
	}

	if (strlen(box_type) == 0) {
		sprintf(fallback_arch_path, "%s/etc/opkg/arch.conf", base_dir);
		nfr4x_log(LOG_DEBUG, "%-33s: fallback to %s parsing", __FUNCTION__, fallback_arch_path);
	...
	if (!strcmp(box_type, box_type_inflash)) {
		nfr4x_log(LOG_DEBUG, "%-33s: box_type_inflash:%s == box_type:%s", __FUNCTION__, box_type_inflash, box_type);
		return 1;
	}
...
nfr4x_device_item *nfr4x_branding_read_info(const char* base_dir, const char *identifier)
{
	...
	settings_value = nfr4x_utils_read(settings_key);

	nfr4x_device_item *item = malloc(sizeof(nfr4x_device_item));
	...
	if (!settings_value) {
		...
		sprintf(distro_cmd, "%s %s %s/usr/lib/enigma2/python image_distro", nfr4x_PYTHON_BIN, nfr4x_BRANDING_HELPER_BIN, base_dir);
		...
		sprintf(version_cmd, "%s %s %s/usr/lib/enigma2/python image_version", nfr4x_PYTHON_BIN, nfr4x_BRANDING_HELPER_BIN, base_dir);
